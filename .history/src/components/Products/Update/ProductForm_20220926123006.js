...
import { isCategoriesValid, isNameValid, isExpiredAfter30Day } from './validators';
...
const ProductForm = (props) => {
    ...
    const [featured, setFeatured] = useState(product.featured === undefined ? false : true);
    const [isInvalidName, setIsInvalidName] = useState(false)
    const [isInvalidCategory, setIsInvalidCategory] = useState(false)
    const [isInvalidExpiration, setIsInvalidExpiration] = useState(false)
    const onSubmit = (e) => {

        if (!isNameValid(name) && !isCategoriesValid(categories)) {
        ...
        <Form onSubmit={onSubmit}>
            <FormGroup>
                ...
                <Input
                    ...
                    placeholder='Name'
                    value={name}
                    onChange={({ target }) => { setName(target.value); setIsInvalidName(!isNameValid(target.value)) }}
                />
            ...
            <FormGroup>
                ...
                <Input
                    invalid={isInvalidExpiration}
                    type="date"
                    name="expirationDate"
                    id="expirationDate"
                    value={expirationDate}
                    onChange={({ target }) => { isInvalidExpiration(isExpiredAfter30Day(target.value)); setExpirationDate(target.value) }}
                />
